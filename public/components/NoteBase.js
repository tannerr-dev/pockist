...
export class NoteBase extends HTMLElement {
	...
	async _showNoteActions(noteId) {
		...
		try {
			...
			case 'share':
				...
				break;
			case 'duplicate':
				await this._doDuplicateNote(noteId);
				break;
			case 'copy-to-list':
				await this._doCopyToList(noteId);
				break;
			case 'convert':
	...
	async _doDuplicateNote(noteId) {
		const newNoteId = await DBManager.duplicateNote(noteId);
		await this._reloadNotes();
		Router.go(`/note/${newNoteId}`);
	}

	async _doCopyToList(noteId) {
		...
		await DBManager.copyNoteToList(noteId, target.id);
	}
